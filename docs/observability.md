...
Kodo Code has one server-side observability model:
...
export T3CODE_OTLP_SERVICE_NAME=kodo-local
```
...
T3CODE_OTLP_SERVICE_NAME=kodo-desktop \
"/Applications/Kodo Code.app/Contents/MacOS/Kodo Code"
```
...
- service name such as `kodo-local`, `t3-dev`, or `kodo-desktop`
...
- `T3CODE_OTLP_SERVICE_NAME`: service name, default `kodo-server`
