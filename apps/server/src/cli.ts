...
const baseDirFlag = Flag.string("base-dir").pipe(
  Flag.withDescription("Base directory path (equivalent to KODOCODE_HOME)."),
  Flag.optional,
);
...
const EnvServerConfig = Config.all({
  ...
  otlpServiceName: Config.string("T3CODE_OTLP_SERVICE_NAME").pipe(
    Config.withDefault("kodo-server"),
  ),
...
const rootCommand = Command.make("kodo", commandFlags).pipe(
  Command.withDescription("Run the Kodo Code server."),
