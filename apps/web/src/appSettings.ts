...
export const AppSettingsSchema = Schema.Struct({
  ...
  askModelSelection: Schema.NullOr(ModelSelection).pipe(withDefaults(() => null)),
  planModelSelection: Schema.NullOr(ModelSelection).pipe(withDefaults(() => null)),
  codeModelSelection: Schema.NullOr(ModelSelection).pipe(withDefaults(() => null)),
  reviewModelSelection: Schema.NullOr(ModelSelection).pipe(withDefaults(() => null)),
...
function normalizeAppSettings(settings: AppSettings): AppSettings {
  const normalizedCustomCodexModels = normalizeCustomModelSlugs(
    settings.customCodexModels,
    "codex",
  );
  const normalizedCustomClaudeModels = normalizeCustomModelSlugs(
    settings.customClaudeModels,
    "claudeAgent",
  );
  const normalizedCustomGeminiModels = normalizeCustomModelSlugs(
    settings.customGeminiModels,
    "gemini",
  );
  const customModelsByProvider = {
    codex: normalizedCustomCodexModels,
    claudeAgent: normalizedCustomClaudeModels,
    gemini: normalizedCustomGeminiModels,
  } satisfies Record<ProviderKind, readonly string[]>;

  return {
    ...settings,
    chatFontSizePx: normalizeChatFontSizePx(settings.chatFontSizePx),
    customCodexModels: normalizedCustomCodexModels,
    customClaudeModels: normalizedCustomClaudeModels,
    customGeminiModels: normalizedCustomGeminiModels,
    askModelSelection: normalizeStoredModeModelSelection(
      settings.askModelSelection,
      customModelsByProvider,
    ),
    planModelSelection: normalizeStoredModeModelSelection(
      settings.planModelSelection,
      customModelsByProvider,
    ),
    codeModelSelection: normalizeStoredModeModelSelection(
      settings.codeModelSelection,
      customModelsByProvider,
    ),
    reviewModelSelection: normalizeStoredModeModelSelection(
...
function normalizeStoredModeModelSelection(
  selection: AppSettings["askModelSelection"],
  customModelsByProvider: Record<ProviderKind, readonly string[]>,
): AppSettings["askModelSelection"] {
  if (!selection) {
    return null;
  }
