...
function buildReconnectTitle(status: WsConnectionStatus): string {
  if (status.nextRetryAt === null) {
    return "Disconnected from Kodo Server";
  }
...
function buildBlockingCopy(
  ...
  if (uiState === "offline") {
    return {
      description:
        "Your browser is offline, so the web client cannot reach the Kodo server. Reconnect to the network and the app will retry automatically.",
      eyebrow: "Offline",
  ...
  if (status.lastError?.trim()) {
    return {
      description: `${status.lastError} Verify that the Kodo server is running and reachable, then reload the app if needed.`,
      eyebrow: "Connection Error",
      title: "Cannot reach the Kodo server",
    };
  }

  return {
    description:
      "The web client could not complete its initial WebSocket connection to the Kodo server. It will keep retrying in the background.",
    eyebrow: "Connection Error",
    title: "Cannot reach the Kodo server",
  };
}
...
function WebSocketBlockingState({
  ...
  return (
    <div className="relative flex min-h-screen w-full items-center justify-center overflow-hidden bg-background px-4 py-10 text-foreground sm:px-6">
      ...
      <section className="relative mx-auto w-full max-w-xl rounded-[1.75rem] border border-border/80 bg-card/92 p-6 shadow-2xl shadow-black/20 backdrop-blur-md sm:p-8">
  ...
  useEffect(() => {
      ...
      const toastPayload = shouldShowOfflineToast
        ...
        : shouldShowExhaustedToast
          ? {
              ...
              timeout: 0,
              title: "Disconnected from Kodo Server",
              type: "error" as const,
              data: {
                hideCopyButton: true,
              },
    ...
    if (
      ...
      const successToast = {
        ...
        title: "Reconnected to Kodo Server",
